...
In this branch I was required to cahnge the user interface developping it on jupyter notebook, starting from the previous branch \textquotesingle{}action\textquotesingle{}. I was also required to add a Doxygen documentation for the package.\hypertarget{md__r_e_a_d_m_e_autotoc_md2}{}\doxysection{Package Composition}\label{md__r_e_a_d_m_e_autotoc_md2}
The package contains the folllowing elements\+:
\begin{DoxyItemize}
\item {\bfseries{action}}\+: folder with .action file to define an action
\item {\bfseries{doc}}\+: folder with Doxygen documentation about the package, generated with doxygen 
...
\item {\bfseries{user\+\_\+interface.\+py}}\+: R\+OS node, implemented as jupyter notebook, to let user command the robot, it has a client for Command service and an action client for Random\+Position service. When user press 1 robot it makes a request to {\itshape \mbox{\hyperlink{state__machine_8cpp}{state\+\_\+machine.\+cpp}}} and robot start moving, otherwise, if he press 0, there is a request for C\+Ommand server to stop the {\itshape \mbox{\hyperlink{state__machine_8cpp}{state\+\_\+machine.\+cpp}}} node, but also an action request {\itshape cancell\+\_\+all\+\_\+goals()} to cancel the goal for the robot and stop it as soon as possible. ~\newline
...
\item Open the user\+\_\+interface in the jupyter notebook and run it (instruction are specified in the notebook) N.\+B. Remeber to source your R\+OS workspace 
\end{DoxyEnumerate}