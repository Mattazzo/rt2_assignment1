...
{\em -\/} & req\+: request for the server
\begin{DoxyItemize}
\item res\+: response from the server
\end{DoxyItemize}\\
\hline
\end{DoxyParams}
...
State\+\_\+machine node which controls the robot, interact with random position service to get a random goal and with position action server to reach it. This node implemtn also a server for user\+\_\+command service, to let user control the robot. variable use to start the robot if it is true 